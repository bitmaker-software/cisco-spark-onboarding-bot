...
module.exports = {
            ...
            models.respondent.find({ where: { email: email } }).then(respondent => {
                if (respondent) {
                    models.respondent_flow.find({
                        where: {
                            respondent_id: respondent.id,
                            respondent_flow_status_id: { $in: [STATUS_TYPES.RESPONDENT_FLOW_STATUS.IN_PROGRESS, STATUS_TYPES.RESPONDENT_FLOW_STATUS.NOT_STARTED] }
    ...
    getStepChoiceAnswersByQuestion: (flowId) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            models.step.findAll({
                attributes: ['id', ['step_order', 'order'],
                    ['text', 'name']
                ],
                where: {
                    flow_id: flowId,
                ...
                res.forEach(function(element) {
                    ...
                    models.step_choice.findAll({
                        ...
                        order: [
                            [models.Sequelize.col('"choice_order"'), 'ASC']
                        ]
                    }).then(options => {
                        //sincronizar
                        let counterOpt = 0;
